...
class GeminiClient:
    ...
    def _post_with_retry(self, http_request: urllib.request.Request) -> str:
        ...
        for attempt in range(self.retries + 1):
            try:
                return self._post_with_requests(http_request)
    ...
    def _post_with_requests(self, http_request: urllib.request.Request) -> str:
        try:
            import requests
        except Exception:
        ...
        try:
            ...
            return self._post_with_curl(http_request)
        if response.status_code >= 400:
            raise RuntimeError(
                f"Gemini endpoint request failed: HTTP {response.status_code}: {response.text}"
            )
        response.encoding = "utf-8"
        return response.text

    def _post_with_curl(self, http_request: urllib.request.Request) -> str:
        command = [
            "curl.exe",
            "-sS",
            "--max-time",
            ...
            http_request.full_url,
            "-H",
            f"Authorization: {http_request.get_header('Authorization')}",
            "-H",
            "Content-Type: application/json",
            "--data-binary",
            "@-",
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"curl fallback failed: {stderr or stdout}")
        return stdout
